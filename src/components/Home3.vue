 ...
 *   1. Check the persistent schema cache; on hit, return a deep clone (no network).
 *   2. Run `query` with { name: typeName } (Apollo cache-first).
 *   3. Bail (return null) if the result lacks the expected `[fieldsKey]` list.
 *   4. Strip __typename recursively.
 *   5. Drop envelope fields whose names are in `envelopeNames`.
 *   6. Upsert the pristine (stripped + filtered, NOT UI-decorated) form into
 *      the schema cache so future mounts skip this whole path.
 *
 * Returns a fresh, safe-to-mutate object. Callers are free to decorate it
 * without worrying about touching the cached pristine copy.
 */
/**
